...
(defn- write-rows
  ([w]
   ...
   (.write w (str " L  SAL                                                      \n")))
  ([w solutions]
    (println (str "write-rows with " (count solutions) " solution"))
    (write-rows w)
    (doall
...
(defn create-mps-file
  [players-data max-salary solutions]
  (let [N (count players-data)]
    (with-open [w (clojure.java.io/writer *mps-file*)]
      (write-meta-data w)
      (write-rows w solutions)
      (write-columns w N players-data solutions)
      (write-rhs w N max-salary solutions)
      (write-bounds w N)
      (.write w (str "ENDATA                                                       \n")))))
...
(defn lpsolve-solve-multiple
  [max-salary players-data solution-count]
  (loop [solutions {}
         count 0]
    (if (>= count solution-count)
      solutions
      (do
        (create-mps-file players-data max-salary solutions)
...
(defn lpsolve-solve-draftkings
  ...
  (let [players-data
    ...
    (print-solutions (lpsolve-solve-multiple constants/*team-salary-draftkings* players-data 5)
                     players-data)))
