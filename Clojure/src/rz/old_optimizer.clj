...
(defn- optimize-fanduel-lineups
  ...
  (let [db (utils/get-db)
    ...
    (nba/lpsolve-solve-fanduel players-proj :linear-projection)))
...
(defn- optimize-fanduel-lineups-svm
  ...
  (let [db (utils/get-db)
    ...
    (nba/lpsolve-solve-fanduel players-proj  :svm-projection)
    )
  )
...
(defn- optimize-fanduel-rtree
  ...
  (let [db (utils/get-db)
    ...
    (data/save-solutions
      (nba/lpsolve-solve-fanduel players-proj  :rtree-projection)
      c/*fanduel*)))
...
(defn- optimize-draftking-lineups
  ...
  (let [db (utils/get-db)
    ...
    (data/save-solutions
      (nba/lpsolve-solve-draftkings players-proj  :linear-projection)
      c/*draftking*)
    ))



(defn- optimize-nhl-draftkings-rtree-ranking
  [prod-run]
  (if prod-run
  ...
  (let [db (utils/get-db)
    ...
    (data/save-solutions
      (nhl/lpsolve-solve-draftkings players-proj  :rank)
      c/*draftking*)
    ))

(defn- random-forest-precision-check
  []
  (let [db (utils/get-db)
        players-data (data/remove-injured
                       (data/filter-roto-suckers
                         (data/filter-unknown db
                                              (data/init-players-data-draftking2))))
        player-names (map :Name players-data)]
    (rtree/precision-check db player-names c/*draftking*)))
...
(defn- optimize-draftking-lineups-avg
  ...
  (let [db (utils/get-db)
    ...
    (data/save-solutions
      (nba/lpsolve-solve-draftkings players-proj  :avg-proj)
      c/*draftking*)))
